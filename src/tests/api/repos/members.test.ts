...
import { createDeleteHandler, createGetHandler } from "@/app/api/repos/[repoId]/members/route";
import { testPrisma } from "@/tests/db";
import { createMockGetCurrentUser, mockEmailProvider } from "@/tests/helpers/deps";
...
describe("GET /api/repos/[repoId]/members", () => {
	it("returns members and pending invitations", async () => {
		...
		mockGetCurrentUser.mockResolvedValueOnce(mockUser({ id: owner.id }));
...
describe("DELETE /api/repos/[repoId]/members", () => {
	...
	it("non-owner cannot remove another member", async () => {
		...
		mockGetCurrentUser.mockResolvedValueOnce(mockUser({ id: memberA.id }));
		const { repository } = await seedRepo({ userId: owner.id });
